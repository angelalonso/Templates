...
fn packet_loop(mut nic: tun_tap::Iface, ih: InterfaceHandle) -> io::Result<()> {
    ...
    loop {
        ...
        // TODO: if self.terminate && Arc get_strong_refs(ih) == 1, then
        // tear down all connections and return
        ...
        // if eth_proto != 0x0800 {
        //     // not IPv4
        //     // TODO: explain what was received
        //     //eprintln!(" WARN >> Received packet of non IPv4 (Ethernet protocol: {})", eth_proto);
        //     continue;
        // }
        //
        ...
        match etherparse::Ipv4HeaderSlice::from_slice(&buf[..nbytes]) {
            Ok(iph) => {
                ...
                if iph.protocol() != 0x06 {
                    ...
                    // not TCP
                    // TODO: explain what was received
                    //eprintln!(" WARN >> Received packet of non TCP (protocol: {})", iph.protocol());
                    continue;
                }
                ...
                match etherparse::TcpHeaderSlice::from_slice(&buf[iph.slice().len()..nbytes]) {
                    Ok(tcph) => {
                        ...
                        let mut cm = &mut *cmg; // cannot borrow cm as mutable more than once at a
                                                // time. cm is a Mutex Guard, so we do Deref and it
                                                // borrows the entire Mutex Guard.
                                                // This solution is a re-borrow
                        let q = Quad {
...
impl TcpListener {
    pub fn accept(&mut self) -> io::Result<TcpStream> {
        ...
        loop {
            ...
            // We keep releasing the block until someone tells us to wait.
